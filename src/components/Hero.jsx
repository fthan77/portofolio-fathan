  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (!del) {
        ...
        setText(w.slice(0, --ci));
        if (ci === 0) { del = false; ri = (ri + 1) % words.length; timer = setTimeout(tick, 100); return; }
      }
      ...
      <div className="relative z-10 max-w-4xl">
        ...
        <div className="border border-outline-variant bg-surface max-w-lg hidden md:block" style={{ fontFamily: "'Space Grotesk', monospace", fontSize: "0.78rem" }}>
          ...
          <div className="p-5" style={{ lineHeight: 1.95 }}>
            {[
              <><span className="text-[#888] mr-5 text-[0.7rem]">1</span><span className="text-[#ff79c6]">const</span> <span className="text-[#8be9fd]">Designer</span> <span className="text-[#aaa]">= () =&gt;</span> {"{"}</>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">2</span>{"  "}<span className="text-[#ff79c6]">const</span> stack = [<span className="text-[#f1fa8c]">'HTML'</span>, <span className="text-[#f1fa8c]">'CSS'</span>, <span className="text-[#f1fa8c]">'JS'</span>, <span className="text-[#f1fa8c]">'React'</span>, <span className="text-[#f1fa8c]">'Tailwind'</span>];</>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">3</span>{"  "}<span className="text-[#ff79c6]">return</span> (</>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">4</span>{"    "}&lt;<span className="text-[#ff5555]">Portfolio</span></>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">5</span>{"      "}<span className="text-[#ffb86c]">name</span>=<span className="text-[#f1fa8c]">"Fathan Fathurohman"</span></>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">6</span>{"      "}<span className="text-[#ffb86c]">role</span>=<span className="text-[#f1fa8c]">"UI/UX Designer"</span></>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">7</span>{"      "}<span className="text-[#ffb86c]">available</span>={"{"}  <span className="text-[#ff79c6]">true</span> {"}"}</>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">8</span>{"    "}/&gt;</>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">9</span>{"  "});</>,
              <><span className="text-[#888] mr-5 text-[0.7rem]">10</span>{"}"}; <span className="text-[#555] italic">// open to work 🔴</span></>,
            ].map((line, i) => (
              <div key={i}>{line}</div>
            ))}
          </div>
